...
import { ProyectoDto, ESTADO_LABELS, EstadoProyecto } from  '../../../shared/models/proyecto.models';
      ...
      <div class="header-right">
        ...
        <div class="header-meta">
          <span class="meta-item">
            ...
            {{ proyecto()!.fechaInicioPlan | date:'dd/MM/yy' }} → {{ proyecto()!.fechaFinPlan | date:'dd/MM/yy' }}
          </span>
          @if (proyecto()!.gerenteProyectoNombre) {
            <span class="meta-item"><i class="pi pi-user"></i> {{ proyecto()!.gerenteProyectoNombre }}</span>
          }
          <span class="meta-item">
            <i class="pi pi-dollar"></i> {{ proyecto()!.presupuestoTotal | currency:'USD':'symbol':'1.0-0' }}
          </span>
        </div>
        <div class="header-actions">
          <!-- NUEVO: Botón Editar proyecto -->
          <button class="btn-sm btn-primary" (click)="abrirEditar()" pTooltip="Editar proyecto">
...
.btn-sm { display:flex; align-items:center; gap:.4rem; padding:.4rem .9rem; border-radius:8px; font-size:.82rem; font-weight:500; text-decoration:none; cursor:pointer; transition:all .2s; border:none; }
...
export class ProyectoDetalleComponent implements OnInit {
  ...
  // ProyectoListDto mínimo para el panel de edición
  readonly proyectoParaEditar = computed(() => {
    const p = this.proyecto();
    if (!p) return null;
    return {
      id: p.id, codigo: p.codigo, nombre: p.nombre,
      tipoProyecto: p.tipoProyecto, estado: p.estado, prioridad: p.prioridad,
      fechaInicioPlan: p.fechaInicioPlan, fechaFinPlan: p.fechaFinPlan,
      porcentajeAvancePlan: p.porcentajeAvancePlan,
      porcentajeAvanceReal: p.porcentajeAvanceReal,
      presupuestoTotal: p.presupuestoTotal, costoRealTotal: p.costoRealTotal,
      gerenteProyectoNombre: p.gerenteProyectoNombre, activo: p.activo,
    };
  });

  // 12 tabs: los 10 originales + Documentos + Recursos
  readonly tabs = [
    ...
    { path: 'recursos',    label: 'Recursos',     icon: 'pi pi-user-edit' },
    ...
    { path: 'documentos',  label: 'Documentos',   icon: 'pi pi-folder' },
  ];
  ...
  ngOnInit(): void {
    ...
    this.service.getById(id).subscribe({
      ...
      error: () => {
        this.toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar el proyecto' });
  ...
  onGuardado(): void {
    ...
    // Recargar proyecto para reflejar cambios en el header
