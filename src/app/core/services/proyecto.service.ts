...
export class ProyectoService {
  ...
  marcarTodasLeidas(proyectoId: number): Observable<void> {
    ...
    crearDocumento(proyectoId: number, req: CrearDocumentoRequest): Observable<ProyectoDocumentoDto> {
    return this.http.post<any>(`${this.base}/proyectos/${proyectoId}/documentos`, req).pipe(map(r => r?.data ?? r));
  }
  getRecursos(proyectoId: number, tareaId?: number): Observable<RecursoProyectoDto[]> {
    let params = new HttpParams();
    if (tareaId) params = params.set('tareaId', tareaId);
    return this.http.get<any>(`${this.base}/proyectos/${proyectoId}/recursos`, { params })
      .pipe(map(r => r?.data ?? r));
  }

  crearRecurso(proyectoId: number, req: CrearRecursoRequest): Observable<RecursoProyectoDto> {
    return this.http.post<any>(`${this.base}/proyectos/${proyectoId}/recursos`, req)
      .pipe(map(r => r?.data ?? r));
  }

  actualizarHorasRecurso(proyectoId: number, recursoId: number, req: ActualizarHorasRecursoRequest): Observable<void> {
    return this.http.patch<void>(`${this.base}/proyectos/${proyectoId}/recursos/${recursoId}/horas`, req);
  }

  eliminarRecurso(proyectoId: number, recursoId: number): Observable<void> {
    return this.http.delete<void>(`${this.base}/proyectos/${proyectoId}/recursos/${recursoId}`);
  }
